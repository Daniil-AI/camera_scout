...
class CameraResearcher:
    def __init__(self, config_path: str = '/reference/base_cam_in_company.json', visualize: bool = True):
        """
        ...
        self.codec_preferences = {'cam': 'MJPG', 
                                  'termal': 'YUYV'}
    ...
    def _load_config(self, path_json: str) -> Dict:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load camera config: check path to reference/base_cam_in_company.json")
        ...
        if self.detailed_info:
            for device in self.detailed_info:
                ...
                if len(device['cam_param']) != 0:
                    ...
                    print(f"• Pixels size: {cam_param['width']}x{cam_param['height']} by {cam_param['fps']} FPS")
                print()
        else:
...
def main():
    Resercher = CameraResearcher(config_path= 'reference/base_cam_in_company.json',
                                 visualize=False)
